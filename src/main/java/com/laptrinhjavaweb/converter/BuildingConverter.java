...
public class BuildingConverter {
	...
	public BuildingSearchResponse convertEntityToBuildingSearchResponse(BuildingEntity buildingEntity) {
		...
		if (!StringUtils.isNullOrEmpty(buildingEntity.getDistrict())) {
	...
	public BuildingSearchBuilder convertSearchRequestToSearchBuilder(BuildingSearchRequest request) {
		BuildingSearchBuilder result = new BuildingSearchBuilder.Builder().setBuildingTypes(request.getBuildingTypes())
				.setDirection(request.getDirection()).setName(request.getName()).setDistrict(request.getDistrict())
				.setFloorArea(request.getFloorArea()).setStreet(request.getStreet()).setWard(request.getWard())
				.setNumberOfBasement(request.getNumberOfBasement()).setRentAreaFrom(request.getRentAreaFrom())
				.setRentAreaTo(request.getRentAreaTo()).setRentPriceFrom(request.getRentPriceFrom())
				.setRentPriceTo(request.getRentPriceTo()).setStaffId(request.getStaffId()).setLevel(request.getLevel())
